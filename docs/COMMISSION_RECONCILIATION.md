...
## Fase 025.5 / 025.5.3 — adaptadores PDF reales, arquitectura definitiva

Con PDF de muestra reales aportados por el ADMIN, se implementaron 3 adaptadores reales usando `pdfjs-dist` para extracción de texto posicional (ver `pdf-table-extract.ts`). **Corrección importante de Fase 025.5.3**: los adaptadores originales estaban nombrados por carrier (`OrangeOscarPdfAdapter`, `OrangeKaiserPdfAdapter`, `EliteBcbsPdfAdapter`) y el selector de la UI mezclaba agencia+modalidad+carrier en una sola opción — esto era incorrecto: Oscar, Kaiser y BCBS son **carriers**, no modalidades de negocio.

**Arquitectura actual (correcta):**

- El selector de fuente (`registry.ts`, `listStatementSources()`) contiene EXACTAMENTE 3 opciones, todas agencia+modalidad, nunca carrier: `ORANGE_OWN` ("Orange — Propias"), `ORANGE_REFERRAL` ("Orange — Referidas"), `ELITE_REFERRAL` ("Elite — Referidas").
- `OrangeOwnPdfAdapter`/`OrangeReferralPdfAdapter` (`orange-own-pdf-adapter.ts`/`orange-referral-pdf-adapter.ts`) son envolturas finas sobre el mismo parser compartido (`orange-pdf-shared.ts::parseOrangeStylePdf`) — el layout de columnas es idéntico entre ambas modalidades, solo cambia qué `businessModality`/`payerAgency` fija cada una. "Member ID" es una columna OPCIONAL detectada automáticamente (variante estructural del layout, nunca una opción de negocio ni un requisito fijo por modalidad).
- `EliteReferralPdfAdapter` (`elite-referral-pdf-adapter.ts`) mantiene su propio parser (layout distinto: `CLIENT DOB`, sin `Status`).
- El **carrier real** (Oscar, Ambetter, Kaiser, BCBS, Cigna...) se **detecta del contenido** de cada fila (columna Carrier/CARRIER) vía `carrier-detection.ts::detectSingleCarrier` — nunca se selecciona en la UI, nunca determina la modalidad. Un archivo con más de un carrier distinto entre sus filas se rechaza por completo (`MultipleCarriersError`, convertido a `VALIDATION_ERROR` en la subida) — un reporte representa un único carrier.
- `reconciliation.service.ts::uploadCommissionStatement` busca el carrier detectado en el catálogo `Carrier` existente (comparación normalizada, insensible a mayúsculas y a sufijos entre paréntesis como "( ACA)") y persiste `CommissionStatement.detectedCarrierName`/`carrierRecognized` — **nunca crea un `Carrier` automáticamente**. `carrierRecognized=false` no bloquea el preview (el ADMIN debe poder revisar qué se detectó) pero sí bloquea `applyCommissionStatement`.
- Agregar soporte a un carrier NUEVO bajo una modalidad ya existente (ej. un reporte de Cigna pagado por Orange como propia) **no requiere código nuevo** — el parser ya lo acepta; solo hace falta que el carrier exista en el catálogo `Carrier` del CRM para que `carrierRecognized` sea `true`.
- La compatibilidad modalidad-vs-clasificación-histórica de la Policy (`matcher.ts::checkModalityCompatibility`) sigue vigente sin cambios: una discrepancia marca la fila `INVALID` para revisión manual, **nunca reclasifica** `Policy.businessSource`.

Ver `pdf-import.service.test.ts` y `pdf-table-extract.test.ts` para la cobertura completa (cantidad de filas variable, múltiples páginas, detección/bloqueo de carrier, separación preview de agencia/modalidad/carrier).
