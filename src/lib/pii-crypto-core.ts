...
// Núcleo del cifrado autenticado recuperable para PII altamente
// sensible (SSN, USCIS/A-Number, números de documento migratorio) —
// Fase 021. Vive SEPARADO de pii-crypto.ts (que reexporta esto con el
// guard "server-only" encima) porque el importador del libro de
// negocio real (Fase 023, scripts/import-book-of-business.ts) corre
// como script de Node/tsx fuera del árbol de Next — "server-only"
// lanza incondicionalmente en ese contexto (no reconoce el bundler),
// así que ese script importa este módulo SIN el guard. Todo el resto
// de la aplicación (Server Components, Server Actions, servicios)
// sigue importando exclusivamente "@/lib/pii-crypto" — el guard sigue
// protegiendo contra un import accidental desde un Client Component.
// La implementación real vive en un solo lugar: aquí.
//
// NUNCA hash irreversible: el negocio necesita poder mostrar el valor
